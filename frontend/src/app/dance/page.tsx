...
const smoothDanceFrames = (
  ...
  return input.map((frame) => {
    if (!frame.pose || frame.pose.length === 0) {
      ...
      return {
        ...frame,
        pose: prevPose.map((point) => point.slice()),
      };
    }
    ...
    prevPose = smoothed.map((point) => point.slice());
    return {
      ...frame,
      pose: smoothed,
    };
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  className={`px-4 py-2 text-sm rounded-md ${
                    isPlaying ? "border border-border" : "bg-text-primary text-white"
